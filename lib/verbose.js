...
const verbose = function () {
  ...
  this.handleStringLiteral = (nextStr, splitSource) => {
    ...
    return literal.replace(/\\/g, "\\\\").replace(/(?<=\(".*)"(?=.*\")/g, '\\"'); // return literal
  };
  ...
  this.handleGroup = (openGlyph, splitSource) => {
    ...
    if (splitSource[0] === "=" || splitSource[0] === "!") {
      ...
      // Add two characters if it is lookbehind
      openGlyphPlusEqual();
      openGlyphPlusEqual();
    }
    ...
    return functonTemplate.clone().fill(methodMap[openGlyph], chainExpression); // fill template with values
  };
